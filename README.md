...
<p align="center">
  <img src="public/icons/icon-192x192.png" alt="ZorMail Logo" width="100" height="100">
  <h1 align="center">ZorMail</h1>
</p>

<p align="center">
  Yo, welcome to <strong>ZorMail</strong> 😎<br/>
  OpenAPI-first temporary email, built for speed, privacy, and one-click Cloudflare deployment.
</p>
...
## Live Demo
[https://ZorMail.app](https://ZorMail.app)

## What Is ZorMail?

ZorMail is a clean temporary-email platform you can self-host on Cloudflare with minimal setup.

- Fast inbox creation and real-time message polling
- Role-based access control for teams
- Webhook notifications and API key support
- Agent-first CLI for automation workflows
- Deploy-ready stack (Next.js + Cloudflare Pages + D1 + KV)
...
## Documentation
**Full Documentation**: [https://docs.ZorMail.app](https://docs.ZorMail.app)
...
```bash
git clone https://github.com/hamzawih0/ZerZor.git
cd ZorMail
```
...
1. Add the following Secrets in GitHub repository settings:
  ...
  - `PROJECT_NAME`: Pages project name (Optional, if empty, defaults to ZorMail)
  - `DATABASE_NAME`: D1 database name (Optional, if empty, defaults to ZorMail-db)
  - `KV_NAMESPACE_NAME`: Cloudflare KV namespace name, used for site settings (Optional, if empty, defaults to ZorMail-kv)
...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/hamzawih0/ZerZor&branch=ZorMail)


## Email Domain Configuration

In the ZorMail User Profile page, you can configure the site's email domains. Supports multiple domain configurations, separated by commas.
...
ZorMail supports sending emails using temporary addresses, based on [Resend](https://resend.com/) service.
...
```json
{
  ...
  "toAddress": "your-email@ZorMail.app"
}
```
...
```json
{
  ...
  "emailDomains": "ZorMail.app,example.com",
...
ZorMail provides an agent-first CLI tool for AI agents and automation workflows.
...
ZorMail config set api-key YOUR_API_KEY

# Create temporary email
ZorMail create --domain ZorMail.app --expiry 1h --json

# Wait for new messages (polling)
ZorMail wait --email-id <id> --timeout 120 --json

# Read message content
ZorMail read --email-id <id> --message-id <id> --json

# Delete email
ZorMail delete --email-id <id>
```
...
Install the built-in skill so AI agents (Claude Code, Codex, etc.) automatically know how to use ZorMail:
...
<table>
  ...
  <tr style="max-width: 360px">
    ...
    <td>
      Add WeChat, remark "ZorMail" to join the WeChat community group.
    </td>
  </tr>
</table>
...
<a href="https://www.star-history.com/#hamzawih0/ZerZor&Date">
 <picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=hamzawih0/ZerZor&type=Date&theme=dark" />
  <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=hamzawih0/ZerZor&type=Date" />
  <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=hamzawih0/ZerZor&type=Date" />
 </picture>
</a>
