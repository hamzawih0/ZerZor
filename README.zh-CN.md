...
<p align="center">
  Yo，欢迎来到 <strong>ZorMail</strong> 😎<br/>
  一个 OpenAPI-first 的临时邮箱项目，主打速度、隐私和 Cloudflare 一键部署。
</p>
...
ZorMail 是一个可以快速自部署到 Cloudflare 的临时邮箱平台，适合个人使用、团队协作和自动化场景。

- 快速创建邮箱 + 实时轮询收件
- 基于角色的权限控制（RBAC）
- 支持 Webhook 通知与 API Key
- 提供面向 Agent 的 CLI 自动化能力
- 完整部署链路（Next.js + Cloudflare Pages + D1 + KV）
...
## 文档
**完整文档**: [https://docs.ZorMail.app](https://docs.ZorMail.app)
...
1. 在 GitHub 仓库设置中添加以下 Secrets：
  ...
  - `CUSTOM_DOMAIN`: 网站自定义域名，用于访问 ZorMail (可选， 如果不填, 则会使用 Cloudflare Pages 默认域名)
  - `PROJECT_NAME`: Pages 项目名 （可选，如果不填，则为 ZorMail） 
  - `DATABASE_NAME`: D1 数据库名称 (可选，如果不填，则为 ZorMail-db)
  - `KV_NAMESPACE_NAME`: Cloudflare KV namespace 名称，用于存储网站配置 （可选，如果不填，则为 ZorMail-kv）
...
在 ZorMail 个人中心页面，可以配置网站的邮箱域名，支持多域名配置，多个域名用逗号分隔
...
ZorMail 支持使用临时邮箱发送邮件，基于 [Resend](https://resend.com/) 服务。
...
ZorMail 提供了专为 AI Agent 设计的命令行工具，用于自动化邮箱工作流。
...
- `CUSTOM_DOMAIN`: 网站自定义域名, 如：ZorMail.app (可选， 如果不填, 则会使用 Cloudflare Pages 默认域名)
- `PROJECT_NAME`: Pages 项目名 （可选，如果不填，则为 ZorMail） 
