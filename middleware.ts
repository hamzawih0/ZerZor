...
export async function middleware(request: Request) {
  ...
  const isAuthOnlyPage = pageSegment === "inbox" || pageSegment === "profile"
  const isGuestOnlyPage = pageSegment === "login"

  if (isAuthOnlyPage || isGuestOnlyPage) {
    const session = await auth()
    if (isAuthOnlyPage && !session?.user) {
      return NextResponse.redirect(new URL(`/${locale}`, request.url))
    }
    if (isGuestOnlyPage && session?.user) {
      return NextResponse.redirect(new URL(`/${locale}`, request.url))
    }
  }
