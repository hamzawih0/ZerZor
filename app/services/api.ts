...
function get<T>(url: string, opts?: { params?: Record<string, string>; responseType?: "blob"; signal?: AbortSignal }) {
...
function parseDomains(emailDomains?: string) {
  return emailDomains
    ? emailDomains.split(",").map((domain) => domain.trim()).filter(Boolean)
    : [];
}
...
const api = {
  getConfig: async () => {
    const [config, emails] = await Promise.all([
      ...
      get<EmailListResponse>("/api/emails"),
    ]);
    ...
    return {
      domains:
        config.domains && config.domains.length > 0
          ? config.domains
          : parseDomains(config.emailDomains),
      emailAddresses:
        config.emailAddresses ?? emails.emails?.map((email) => email.address) ?? [],
    };
  },
  getSession: () => get<SessionInfo>("/api/v1/session"),
  ...
  createMailbox: (email: string, name: string, settings?: unknown) => {
    ...
    if (!settings && domain) {
      return post<{ id: string; email: string }>("/api/emails/generate", {
        name: name || localPart || email,
        expiryTime: 0,
        domain,
      });
    }

    return post<Mailbox>("/api/v1/mailboxes", { email, name, settings });
  },
