...
export default function HomeRoute() {
	...
	return (
		<div className="home-shell min-h-screen bg-kumo-recessed">
			<AsciiWavesBackground />
			<div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12">
				<div className="mb-8 rounded-[28px] border border-kumo-line bg-kumo-base/90 backdrop-blur-xl p-6 md:p-8 shadow-[0_20px_70px_rgba(0,0,0,0.18)]">
					<div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
						<div className="max-w-2xl space-y-4">
							<div className="flex items-center gap-2">
								<ThemeToggle />
								<span className="text-xs uppercase tracking-[0.25em] text-kumo-subtle">ZorMail control hub</span>
							</div>
							<h1 className="text-3xl font-bold text-kumo-default md:text-5xl">Advanced mail operations, inboxes, and controls.</h1>
							<p className="max-w-2xl text-sm leading-6 text-kumo-subtle md:text-base">
								Manage mailboxes, routing, appearance, access, and delivery from one production-ready control surface.
							</p>
						</div>
						<div className="flex flex-wrap gap-2">
							<Button variant="primary" onClick={() => navigate("/admin")}>Open admin center</Button>
							<Button variant="secondary" onClick={() => navigate("/auth")}>Sign in / sign up</Button>
							{!isConfigured && (
								<Button variant="ghost" onClick={() => setIsCreateOpen(true)}>Create mailbox</Button>
							)}
						</div>
					</div>
					<div className="mt-6 grid gap-3 sm:grid-cols-3">
						<div className="rounded-2xl border border-kumo-line bg-kumo-recessed/70 p-4">
							<div className="text-xs uppercase tracking-widest text-kumo-subtle">Mailboxes</div>
							<div className="mt-2 text-2xl font-semibold text-kumo-default">{mailboxes.length}</div>
						</div>
						<div className="rounded-2xl border border-kumo-line bg-kumo-recessed/70 p-4">
							<div className="text-xs uppercase tracking-widest text-kumo-subtle">Domains</div>
							<div className="mt-2 text-2xl font-semibold text-kumo-default">{domains.length || 0}</div>
						</div>
						<div className="rounded-2xl border border-kumo-line bg-kumo-recessed/70 p-4">
							<div className="text-xs uppercase tracking-widest text-kumo-subtle">Access</div>
							<div className="mt-2 text-sm font-medium text-kumo-default">Cloudflare Access ready</div>
						</div>
					</div>
				</div>
				<div className="mb-8">
